...
export default function StockChart() {
  ...
  useEffect(() => {
    const currentInfo =
      (isFavorite ? favoriteDisplayInfo : defaultDisplayInfo) || [];
    setCurrentDisplayInfo(currentInfo);
    const hasError = isFavorite ? favoriteIsError : defaultIsError;
    setHasData(
      !(
        (hasError || !currentInfo || currentInfo.length === 0) &&
        !favoriteIsLoading &&
        !defaultIsLoading
      )
    );
  ...
  useEffect(() => {
    if (hasData) {
      const labels = currentDisplayInfo.map((info) =>
        format(new Date(info.IntervalStart), "yyyy-MM-dd")
      );
      ...
      const datasets = stockNames.map((stockName) => {
        const stockData = currentDisplayInfo.filter(
          (info) => info.StockName === stockName
        );
